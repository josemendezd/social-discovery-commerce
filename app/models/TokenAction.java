...
public class TokenAction extends Model {
	...
	{
		return create(type, token, targetUser, VERIFICATION_TIME);
	}

	public static TokenAction create(final Type type, final String token,
			final User targetUser,long verificationtimelimit) {
		...
		ua.created = created;
		ua.expires = new Date(created.getTime() + verificationtimelimit * 1000);
		ua.save();
		return ua;
	}
}
